...
// generates RSA key pair for the host
// the idea is to use the host's public key to encrypt a random AES key generated by the client
// the client uses the AES key, encrypts with the host's public and sends on the first connection
func generateRSAKeyPair() error {
	dir, err := getLighthouseDir()
	if err != nil {
		return fmt.Errorf("failed to get lighthouse directory: %w", err)
	}
	keysDir := filepath.Join(dir, "keys") // gets the keys dir, otherwise creates it if it doesn't exist
	...
	// if keys already exist, skip generation
	...
	// this key struct contains both private and public key on key.PrivateKey and key.PublicKey
